...
#load data
library(rio)
data=import("C:\\Users\\haozi\\OneDrive\\Desktop\\PhD\\fMRI_PrC-PPC_data\\plots_abovethreshold_pre20230328\\manual_results2023.xlsx")
#output folder
...
p1 <- ggplot(PrC_test_rele_data, aes(x=signal, y=beta_avg, fill=ROI)) + 
  ...
  geom_errorbar(aes(ymin=beta_avg-beta_sd, ymax=beta_avg+beta_sd), width=.2,position=position_dodge(.9),colour="white")+ 
...
ggsave(filename='PrC_task-rele_life-recent-conj_bar_sd.png',path=datapath,plot=p1,scale = 0.9,width = 2400,height=1600,units='px')
...
ggsave(filename='PrC_task-rele_life-conj_bar_sd.png',path=datapath,plot=p2.1,width = 350*3,height=467*3,units='px',scale = 0.9)
#frequency judgement
...
ggsave(filename='PrC_task-irrele_life-conj_bar_sd.png',path=datapath,plot=p2.2,width = 350*3,height=467*3,units='px',scale = 0.9)
...
ggsave(filename='PrC_task-irrele_recent_bar_sd.png',path=datapath,plot=p3,width = 2400,height=1600,units='px',scale = 0.9)
...
p4 <- ggplot(study_lifetime_all_data, aes(x=signal, y=beta_avg, fill=ROI)) + 
  ...
  theme(plot.background = element_rect(fill="black"),panel.background = element_rect(fill="black", colour="white"),panel.border = element_blank(),
        ...
        axis.title = element_blank())+ 
  labs(x="Task-irrelevant lifetime familiarity", y ="Parameter estimates")+scale_x_discrete(labels=c('1','2','3','4','5'))
ggsave(filename='PrC_study_life-all_bar_sd.png',path=datapath,plot=p4,width = 2400,height=1600,units='px',scale = 0.9)
...
p5 <- ggplot(study_lifetime_pres1_data, aes(x=signal, y=beta_avg, fill=ROI)) + 
  ...
  theme(plot.background = element_rect(fill="black"),panel.background = element_rect(fill="black", colour="white"),panel.border = element_blank(),
        ...
        axis.title = element_blank())+ 
  labs(x="Task-irrelevant lifetime familiarity", y ="Parameter estimates")+scale_x_discrete(labels=c('1','2','3','4','5'))
ggsave(filename='PrC_study_life-pres1_bar_sd.png',path=datapath,plot=p5,width = 2400,height=1600,units='px',scale = 0.9)
