...
function test_1stlvl(project_derivative,output,sub,expstart_vol,fmriprep_foldername,TR,noresp_opt)
...
%% step 1 generate alltrial regressor and noise regressor
        ...
        temp_dir=strcat(sub_dir,'/temp/');
            ...
            % 20190929 remember to change the task name to
            % "test" for actual participants
...
%                 run_temp=strcat(temp_dir,strcat(task{1},erase(run{1},'_')));
                ...
                prefix(:)={substr.runsmooth{j}};%should be a unique run name
                %prefix=prefix';
