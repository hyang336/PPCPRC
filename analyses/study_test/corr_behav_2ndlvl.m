...
switch effect
    case 'priming'
        %% freq
        %extract contrast
        freq_lvl1_folder=strcat(project_derivative,'/GLM_avgMask_4mmSmooth/repetition_suppression_softAROMA/');
        %prc mask
        freq_maskfile=strcat(project_derivative,'/masks/abovethreshold_PrC_masks/studyphase_pres1vs789_dec/lPrC75_SVC_abovethreshold_mask.nii');
        freq_mask=niftiread(freq_maskfile);
        %% fam
        %extract contrast
        fam_lvl1_folder=strcat(project_derivative,'/GLM_avgMask_pscan020022exclude/postscan_lifetime_softAROMA_const-epoch/');
        %prc mask
        fam_maskfile=strcat(project_derivative,'/masks/abovethreshold_PrC_masks/studyphase_lifetime_alltrial/lPrC75_SVC_abovethreshold_mask.nii');
        fam_mask=niftiread(fam_maskfile);
        %%
        %calculate behavioral variable
        behav_folder=strcat(project_derivative,'/behavioral/');
        freq_prime_res=cell(0,3);
        fam_prime_res=cell(0,3);
        for i=1:length(SSID)
            freqconimg=niftiread(strcat(freq_lvl1_folder,'/sub-',SSID{i},'/temp/con_0004.nii'));%con_0004 is the pres1vs789 contrast
            freq_prc_con_val=nanmean(freqconimg(find(freq_mask)));
            
            famconimg=niftiread(strcat(fam_lvl1_folder,'/sub-',SSID{i},'/temp/con_0001.nii'));%con_0001 is the study lifetime dec
            fam_prc_con_val=nanmean(famconimg(find(fam_mask)));
            %post-scan beh
            ...
            %regress RT on lifetime and freq (both z-scored) and extract slopes, for the two outliers
            %need to use normative data
            if ~ismember(SSID{i},{'020','022'})
                b=regress(cell2mat(event(:,7)),[ones(size(event(:,7))),mod(cell2mat(event(:,2)),10),str2num(cell2mat(event(:,13)))]);
            else
                b=regress(cell2mat(event(:,7)),[ones(size(event(:,7))),mod(cell2mat(event(:,2)),10),cell2mat(event(:,3))]);
            end
            
            %compile results
            freq_prime_res(i,1)=SSID(i);
            freq_prime_res(i,2)={freq_prc_con_val};
            freq_prime_res(i,3)={b(2)};
            
            fam_prime_res(i,1)=SSID(i);
            fam_prime_res(i,2)={fam_prc_con_val};
            fam_prime_res(i,3)={b(3)};
        end
        
        %correlate across participants
        [r1,p1]=corrcoef(cell2mat(freq_prime_res(:,2)),cell2mat(freq_prime_res(:,3)));
        [r2,p2]=corrcoef(cell2mat(fam_prime_res(:,2)),cell2mat(fam_prime_res(:,3)));
    ...
    case 'priming_freq'
        ...
        for i=1:length(SSID)
            ...
            b1=[ones(size(event(:,2))),mod(cell2mat(event(:,2)),10)]\cell2mat(event(:,7));
    ...
    case 'priming_fam'
        ...
        for i=1:length(SSID)
            ...
            %need to use normative data
            if ~ismember(SSID{i},{'020','022'})
                b1=[ones(size(event(:,13))),str2num(cell2mat(event(:,13)))]\cell2mat(event(:,7));
            else
                b1=[ones(size(event(:,3))),cell2mat(event(:,3))]\cell2mat(event(:,7));
            end
    ...
    case 'mirror'
        ...
        for i=1:length(SSID)
            ...
            if ~ismember(SSID{i},{'020','022'})
                b1=[ones(size(event(:,13))),str2num(cell2mat(event(:,13)))]\freq_error;
            else
                b1=[ones(size(event(:,3))),cell2mat(event(:,3))]\freq_error;
            end
