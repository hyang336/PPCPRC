%% center around the mean of subject scale usage for each task, then test for high vs. low recent/lifetime
...
function test_resp_1stlvl_mean_centered_di(project_derivative,output,sub,expstart_vol,fmriprep_foldername,TR,noresp_opt,maskfile)
...
sub_dir=strcat(output,'/test_1stlvl_mean_centered_di/',sub);
...
switch noresp_opt
    case 'regress'
            ...
            for j=1:length(substr.run)
                task=regexp(substr.run{j},'task-\w*_','match');
                run=regexp(substr.run{j},'run-\d\d_','match');
                events{j}=load_event_test(project_derivative,sub,task,run,expstart_vol,TR);
            end
            %concatenate the 4 runs
            catevents=vertcat(events{1},events{2},events{3},events{4});
...
%                 run_temp=strcat(temp_dir,strcat(task{1},erase(run{1},'_')));
                ...
                cond={'recent_low','recent_high','lifetime_low','lifetime_high','noresp';recent_low,recent_high,lifetime_low,lifetime_high,noresp};
...
%% load SPM.mat and use the design matrix info to define contrasts
                ...
                interact_col(1,2:5,1)={'life1ow_fo_col_in_spmmat','lifehigh_fo_col_in_spmmat','recentlow_fo_col_in_spmmat','recenthigh_fo_col_in_spmmat'};
                empty_col=cell(1);
                for l=1:length(substr.run)
                    ...
                    [~,interact_col{l+1,3,1}]=find(strcmp(strcat('Sn(',num2str(l),') lifetime_highxfeat_over^1*bf(1)'),spmmat.SPM.xX.name(1,:)));

                    [~,interact_col{l+1,4,1}]=find(strcmp(strcat('Sn(',num2str(l),') recent_lowxfeat_over^1*bf(1)'),spmmat.SPM.xX.name(1,:)));
                    [~,interact_col{l+1,5,1}]=find(strcmp(strcat('Sn(',num2str(l),') recent_highxfeat_over^1*bf(1)'),spmmat.SPM.xX.name(1,:)));

                    %indicate in the 3rd dimension if any of
                    %the above columns are all zeros
                    for m=2:size(interact_col,2)
                ...
                %right columns
                [~,lifelow_main_col]=find(contains(spmmat.SPM.xX.name(1,:),'lifetime_low*bf(1)'));
                [~,lifehigh_main_col]=find(contains(spmmat.SPM.xX.name(1,:),'lifetime_high*bf(1)'));
                ...
                convec(1,lifelow_main_col)=-1/length(lifelow_main_col);
                convec(1,lifehigh_main_col)=1/length(lifehigh_main_col);
                ...
                [~,recentlow_main_col]=find(contains(spmmat.SPM.xX.name(1,:),'recent_low*bf(1)'));
                [~,recenthigh_main_col]=find(contains(spmmat.SPM.xX.name(1,:),'recent_high*bf(1)'));
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,recentlow_main_col)=1/length(recentlow_main_col);         
                convec(1,recenthigh_main_col)=-1/length(recenthigh_main_col);
                ...
                [~,lifelow_fomod_col]=find(contains(spmmat.SPM.xX.name(1,:),'lifetime_lowxfeat_over^1*bf(1)'));
                [~,lifehigh_fomod_col]=find(contains(spmmat.SPM.xX.name(1,:),'lifetime_highxfeat_over^1*bf(1)'));

                %remove all zero columns
                for n=1:length(empty_col)
                   lifelow_fomod_col=lifelow_fomod_col(lifelow_fomod_col~=empty_col{n});
                   lifehigh_fomod_col=lifehigh_fomod_col(lifehigh_fomod_col~=empty_col{n});
                end
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,lifelow_fomod_col)=-1/length(lifelow_fomod_col);
                convec(1,lifehigh_fomod_col)=1/length(lifehigh_fomod_col);
                ...
                [~,recentlow_fomod_col]=find(contains(spmmat.SPM.xX.name(1,:),'recent_lowxfeat_over^1*bf(1)'));
                [~,recenthigh_fomod_col]=find(contains(spmmat.SPM.xX.name(1,:),'recent_highxfeat_over^1*bf(1)'));

                %remove all zero columns
                for n=1:length(empty_col)
                   recentlow_fomod_col=recentlow_fomod_col(recentlow_fomod_col~=empty_col{n});
                   recenthigh_fomod_col=recenthigh_fomod_col(recenthigh_fomod_col~=empty_col{n});
                end
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,recentlow_fomod_col)=1/length(recentlow_fomod_col);
                convec(1,recenthigh_fomod_col)=-1/length(recenthigh_fomod_col);
                ...
                sum_life_mod=length(lifelow_fomod_col)+length(lifehigh_fomod_col);
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,lifelow_fomod_col)=1/sum_life_mod;
                convec(1,lifehigh_fomod_col)=1/sum_life_mod;
                ...
                %runs
                sum_recent_mod=length(recentlow_fomod_col)+length(recenthigh_fomod_col);
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,recentlow_fomod_col)=1/sum_recent_mod;
                convec(1,recenthigh_fomod_col)=1/sum_recent_mod;
                ...
                convec=zeros(1,length(spmmat.SPM.xX.name(1,:)));%contrast vector should be of the same dimension as the number of columns in the design matrix
                convec(1,lifelow_fomod_col)=1/sum_all_mod;
                convec(1,lifehigh_fomod_col)=1/sum_all_mod;
                convec(1,recentlow_fomod_col)=1/sum_all_mod;
                convec(1,recenthigh_fomod_col)=1/sum_all_mod;
