...
########################################################################################################################################################
    ...
    #save trace
    az.to_netcdf(infer_data_race4nba_v,outdir +'sample_' + str(burnin) + '_' + str(samples) + '_trace_Fixed_az_' + signalname + '_' + modelname + '_bin' + bin_ver + '.nc4')
    #save trace plot
    az.plot_trace(
        infer_data_race4nba_v,
        var_names="~log_likelihood",  # we exclude the log_likelihood traces here
    )
    plt.savefig(outdir+'posterior_diagnostic_' + str(burnin) + '_' + str(samples) + '_trace_Fixed_az_' + signalname + '_' + modelname + '_bin' + bin_ver + '.png')
    #save summary
    res_sum=az.summary(model.traces)
    res_sum.to_csv(outdir+'summary_' + str(burnin) + '_' + str(samples) + '_trace_Fixed_az_' + signalname + '_' + modelname + '_bin' + bin_ver + '.csv')