...
%% also binarize the responses around median response option and output a spreadsheet for HDDM/HSSM
%% pass in different ROI mask to extract betas in different regions

function HSSM_feature_selection_Medianbin(sublist,LSSN_foldername,mask_dir,project_derivative,output_dir)
...
for i=1:length(SSID)
    ...
    % extract median freq rating and count for each bin
    median_freq=median(freq_ratings);
    %matlab calculate median as the mean of middle 2 numbers, need to
    %handle cases when that is not an integer
    if mod(median_freq,1)~=0
        median_freq=ceil(median_freq);
    end
    disp(median_freq);
    ...
    freq_count=[size(freq_trials_resp(strcmp(freq_trials_resp(:,6),'1'),7),1),...
        ...
        left_count = sum(freq_count(1:median_freq-1));
   
        right_count = sum(freq_count(median_freq+1:end));
    ...
    %calculate which side has few trials, and record that as a variable
        ...
        if left_count<=right_count
            ...
            low_resp=[1:1:median_freq];
            high_resp=[median_freq+1:1:5];
        else
            freq_bin_scheme='right';
            low_resp=[1:1:median_freq-1];
            high_resp=[median_freq:1:5];
        end
    ...
    % extract median lifetime rating and count for each bin
    median_life=median(life_ratings);
    if mod(median_life,1)~=0
        median_life=ceil(median_life);
    end
    disp(median_life);
    life_count=[size(life_trials_resp(strcmp(life_trials_resp(:,6),'1'),7),1),...
        ...
        left_count = sum(life_count(1:median_life-1));
   
        right_count = sum(life_count(median_life+1:end));
    ...
    %calculate which side has few trials, and record that as a variable
        ...
        if left_count<=right_count
            ...
            low_resp=[1:1:median_life];
            high_resp=[median_life+1:1:5];
        else
            life_bin_scheme='right';
            low_resp=[1:1:median_life-1];
            high_resp=[median_life:1:5];
        end
...
writetable(freq_result,strcat(output_dir,'/HSSM_freq_MedianBin_data.csv'));
writetable(life_result,strcat(output_dir,'/HSSM_life_MedianBin_data.csv'));
end
